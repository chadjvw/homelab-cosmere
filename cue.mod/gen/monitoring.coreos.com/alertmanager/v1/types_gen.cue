...
//timoni:generate timoni mod vendor crd -f https://github.com/prometheus-community/helm-charts/raw/refs/heads/main/charts/kube-prometheus-stack/charts/crds/crds/crd-alertmanagers.yaml
...
// The `Alertmanager` custom resource definition (CRD) defines a
// desired [Alertmanager](https://prometheus.io/docs/alerting)
// setup to run in a Kubernetes cluster. It allows to specify
// many options such as the number of replicas, persistent
// storage and many more.
//
// For each `Alertmanager` resource, the Operator deploys a
// `StatefulSet` in the same namespace. When there are two or
// more configured replicas, the Operator runs the Alertmanager
// instances in high-availability mode.
//
// The resource defines via label and namespace selectors which
// `AlertmanagerConfig` objects should be associated to the
// deployed Alertmanager instances.
#Alertmanager: {
...
#AlertmanagerSpec: {
	...
	// passed as-is to the
	// Alertmanager container which may cause issues if they are
	...
	alertmanagerConfigMatcherStrategy?: {
		// AlertmanagerConfigMatcherStrategyType defines the strategy used
		// by
		// AlertmanagerConfig objects to match alerts in the routes and
		// inhibition
		// rules.
		//
	...
	alertmanagerConfiguration?: {
		...
		global?: {
			...
			// alert does
			// not include EndsAt, after this time passes it can declare the
			// alert as resolved if it has not been updated.
			// This has no impact on alerts from Prometheus, as they always
			// include EndsAt.
		...
		// generate the Alertmanager configuration.
		// It must be defined in the same namespace as the Alertmanager
		// object.
	...
	// If the service account has `automountServiceAccountToken:
	// true`, set the field to `false` to opt out of automounting API
	// credentials.
	...
	// You should only set it when the Alertmanager cluster includes
	// Alertmanager instances which are external to this Alertmanager
	// resource. In practice, the addresses of the external instances
	// are provided via the `.spec.additionalPeers` field.
	clusterLabel?: string
	...
	// Configures the mutual TLS configuration for the Alertmanager
	// cluster's gossip protocol.
	//
	...
	// namespace as the
	// Alertmanager object, which contains the configuration for this
	// Alertmanager
	// instance. If empty, it defaults to
	...
	// the
	// operator provisions a minimal Alertmanager configuration with
	// one empty
	...
	// the scope
	// of what the maintainers will support and by doing so, you
	// accept that
	...
	// Enable access to Alertmanager feature flags. By default, no
	...
	// under. This is
	// necessary to generate correct URLs. This is necessary if
	// Alertmanager is not
	// served from root of a DNS name.
	externalUrl?: string

	// ForceEnableClusterMode ensures Alertmanager does not deactivate
	// the cluster mode when running with a single replica.
	// Use case is e.g. spanning an Alertmanager cluster across
	// Kubernetes clusters with a single replica in each.
	...
	// definition. Those can be used to e.g.
	// fetch secrets for injection into the Alertmanager configuration
	// from external sources. Any
	// errors during the execution of an initContainer will lead to a
	// restart of the Pod. More info:
	// https://kubernetes.io/docs/concepts/workloads/pods/init-containers/
	// InitContainers described here modify an operator
	// generated init containers if they share the same name and
	// modifications are
	// done via a strategic merge patch. The current init container
	// name is:
	// `init-config-reloader`. Overriding init containers is entirely
	// outside the
	// scope of what the maintainers will support and by doing so, you
	// accept that
	// this behaviour may break at any time without notice.
	...
	// Defines the limits command line flags when starting
	// Alertmanager.
	limits?: {
		// The maximum size of an individual silence as stored on disk.
		// This corresponds to the Alertmanager's
		// `--silences.max-per-silence-bytes` flag.
		// It requires Alertmanager >= v0.28.0.
		maxPerSilenceBytes?: =~"(^0|([0-9]*[.])?[0-9]+((K|M|G|T|E|P)i?)?B)$"

		// The maximum number active and pending silences. This
		// corresponds to the
	...
	// so that it
	// does not bind against the Pod IP. Note this is only for the
	// Alertmanager
	...
	// * "app.kubernetes.io/version" label, set to the Alertmanager
	// version.
	...
	// controller will
	// eventually make the size of the running cluster equal to the
	// expected
	// size.
	replicas?: int32
	...
	// '120h',
	// and must match the regular expression `[0-9]+(ms|s|m|h)`
	// (milliseconds seconds minutes hours).
	retention?: =~"^(0|(([0-9]+)h)?(([0-9]+)m)?(([0-9]+)s)?(([0-9]+)ms)?)$"

	// The route prefix Alertmanager registers HTTP handlers for. This
	// is useful,
	// if using ExternalURL and a proxy is rewriting HTTP routes of a
	// request,
	// and the actual ExternalURL is still true, but the server serves
	// requests
	...
	// If defined, the Service must be created before the Alertmanager
	// resource in the same namespace and it must define a selector
	// that matches the pod labels.
	// If empty, the operator will create and manage a headless
	// service named `alertmanager-operated` for Alermanager
	// resources.
	// When deploying multiple Alertmanager resources in the same
	// namespace, it is recommended to specify a different value for
	// each.
	// See
	...
	// Similar to a tag, but the SHA explicitly deploys an immutable
	// container image.
	...
	// output StatefulSet definition.
	// Volumes specified will be appended to other volumes that are
