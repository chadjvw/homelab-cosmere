...
//timoni:generate timoni mod vendor crd -f https://github.com/prometheus-community/helm-charts/raw/refs/heads/main/charts/kube-prometheus-stack/charts/crds/crds/crd-podmonitors.yaml

package v1

import "strings"

// The `PodMonitor` custom resource definition (CRD) defines how
// `Prometheus` and `PrometheusAgent` can scrape metrics from a
// group of pods.
...
//
// `Prometheus` and `PrometheusAgent` objects select `PodMonitor`
...
#PodMonitorSpec: {
	...
	// metrics
	// defaults to the namespace and name of the PodMonitor object
	// (e.g. `<namespace>/<name>`).
	jobLabel?: string
	...
	// should discover the pods.
	// By default, the pods are discovered in the same namespace as
	// the `PodMonitor` object but it is possible to select pods
	...
	podMetricsEndpoints?: [...{
		...
		// namespace
		// as the PodMonitor object and readable by the Prometheus
		// Operator.
		//
		...
		//
		// It takes precedence over the `portNumber` and `targetPort`
		// fields.
		port?: string
	...
	// Label selector to select the Kubernetes `Pod` objects to scrape
	// metrics from.
	selector!: {
