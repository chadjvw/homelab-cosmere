...
//timoni:generate timoni mod vendor crd -f https://github.com/prometheus-community/helm-charts/raw/refs/heads/main/charts/kube-prometheus-stack/charts/crds/crds/crd-thanosrulers.yaml
...
// The `ThanosRuler` custom resource definition (CRD) defines a
// desired [Thanos
// Ruler](https://github.com/thanos-io/thanos/blob/main/docs/components/rule.md)
// setup to run in a Kubernetes cluster.
//
// A `ThanosRuler` instance requires at least one compatible
// Prometheus API endpoint (either Thanos Querier or Prometheus
// services).
//
// The resource defines via label and namespace selectors which
// `PrometheusRule` objects should be associated to the deployed
// Thanos Ruler instances.
#ThanosRuler: {
...
#ThanosRulerSpec: {
	...
	// Configures the label names which should be dropped in Thanos
	// Ruler
	// alerts.
	//
	// The replica label `thanos_ruler_replica` will always be dropped
	// from the alerts.
	...
	//
	// Alert relabel configuration must have the form as specified in
	// the
	...
	// Configures the list of Alertmanager endpoints to send alerts
	// to.
	//
	// The configuration format is defined at
	...
	// operator generated
	// containers. This can be used to allow adding an authentication
	// proxy to a ThanosRuler pod or
	// to change the behavior of an operator generated container.
	// Containers described here modify
	// an operator generated container if they share the same name and
	// modifications are done via a
	// strategic merge patch. The current container names are:
	// `thanos-ruler` and `config-reloader`.
	// Overriding containers is entirely outside the scope of what the
	// maintainers will support and by doing
	...
	// Enable access to Thanos Ruler feature flags. By default, no
	...
	// origin for each alert
	// and metric that is user created. The label value will always be
	// the namespace of the object that is
	// being created.
	...
	// under. This is
	// necessary to generate correct URLs. This is necessary if Thanos
	// Ruler is not
	// served from root of a DNS name.
	externalPrefix?: string

	// GRPCServerTLSConfig configures the gRPC server from which
	// Thanos Querier reads
	// recorded rule data.
	// Note: Currently only the CAFile, CertFile, and KeyFile fields
	// are supported.
	...
	// fetch secrets for injection into the ThanosRuler configuration
	...
	// fetching is entirely outside the scope
	// of what the maintainers will support and by doing so, you
	// accept that this behaviour may break
	...
	// Configures the external label pairs of the ThanosRuler
	// resource.
	//
	// A default replica label `thanos_ruler_replica` will be always
	// added as a
	...
	//
	// The configuration format is defined at
	// https://thanos.io/tip/thanos/storage.md/#configuring-access-to-object-storage
	//
	// The operator performs no validation of the configuration.
	//
	...
	// * "thanos-ruler" label, set to the name of the ThanosRuler
	// instance.
	...
	// to be excluded from enforcing
	// of adding namespace labels. Works only if
	...
	// Configures the list of Thanos Query endpoints from which to
	// query metrics.
	//
	// The configuration format is defined at
	...
	// query metrics.
	//
	// For Thanos >= v0.11.0, it is recommended to use `queryConfig`
	// instead.
	//
	...
	//
	// When the list isn't empty, the ruler is configured with
	// stateless mode.
	//
	...
	// '24h', and
	// must match the regular expression `[0-9]+(ms|s|m|h|d|w|y)`
	// (milliseconds
	// seconds minutes hours days weeks years).
	//
	// The field has no effect when remote-write is configured since
	// the Ruler
	...
	ruleConcurrentEval?: int32 & >=1

	// Minimum duration between alert and restored "for" state.
	// This is maintained only for alerts with configured "for" time
	...
	// If defined, the Service must be created before the ThanosRuler
	...
	// service named `thanos-ruler-operated` for ThanosRuler
	// resources.
	// When deploying multiple ThanosRuler resources in the same
	...
	// output StatefulSet definition. Volumes specified will
	// be appended to other volumes that are generated as a result of
	// StorageSpec objects.
	volumes?: [...{
