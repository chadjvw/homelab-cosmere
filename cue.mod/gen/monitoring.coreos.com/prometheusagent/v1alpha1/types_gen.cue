...
//timoni:generate timoni mod vendor crd -f https://github.com/prometheus-community/helm-charts/raw/refs/heads/main/charts/kube-prometheus-stack/charts/crds/crds/crd-prometheusagents.yaml
...
// The `PrometheusAgent` custom resource definition (CRD) defines
// a desired [Prometheus
// Agent](https://prometheus.io/blog/2021/11/16/agent/) setup to
// run in a Kubernetes cluster.
//
// The CRD is very similar to the `Prometheus` CRD except for
// features which aren't available in agent mode like rule
// evaluation, persistent storage and Thanos sidecar.
#PrometheusAgent: {
...
#PrometheusAgentSpec: {
	...
	// Mode defines how the Prometheus operator deploys the
	// PrometheusAgent pod(s).
	//
	// (Alpha) Using this field requires the
	// `PrometheusAgentDaemonSet` feature gate to be enabled.
