...
#VolumeSnapshotContent: {
	...
	// the underlying storage system. Required.
...
#VolumeSnapshotContentSpec: {
	...
	// Supported values are "Retain" and "Delete". "Retain" means
	// that the VolumeSnapshotContent and its physical snapshot on
	// underlying storage system are kept. "Delete" means that the
	// VolumeSnapshotContent and its physical snapshot on underlying
	// storage system are deleted. For dynamically provisioned
	// snapshots, this field will automatically be filled in by the
	// CSI snapshotter sidecar with the "DeletionPolicy" field
	// defined in the corresponding VolumeSnapshotClass. For
	// pre-existing snapshots, users MUST specify this field when
	// creating the VolumeSnapshotContent object. Required.
	deletionPolicy!: "Delete" | "Retain"

	// driver is the name of the CSI driver used to create the
	// physical snapshot on the underlying storage system. This MUST
	// be the same as the name returned by the CSI GetPluginName()
	...
	// Kubernetes object representation. This field is immutable
	// after creation. Required.
	source!: {
		// snapshotHandle specifies the CSI "snapshot_id" of a
		// pre-existing snapshot on the underlying storage system for
	...
	// (or will be) created. Note that after provisioning, the
	// VolumeSnapshotClass may be deleted or recreated with different
	// set of values, and as such, should not be referenced
	// post-snapshot creation.
	volumeSnapshotClassName?: string

	// volumeSnapshotRef specifies the VolumeSnapshot object to which
	// this VolumeSnapshotContent object is bound.
	// VolumeSnapshot.Spec.VolumeSnapshotContentName field must
	// reference to this VolumeSnapshotContent's name for the
	...
	volumeSnapshotRef!: {
		...
		// well-defined way of referencing a part of an object. TODO:
		// this design is not final and this field is subject to change
		// in the future.
		fieldPath?: string
