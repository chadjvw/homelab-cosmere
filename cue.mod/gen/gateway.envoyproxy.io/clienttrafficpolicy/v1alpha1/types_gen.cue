...
#ClientTrafficPolicySpec: {
	...
	connection?: {
		...
		// MaxAcceptPerSocketEvent provides configuration for the maximum
		// number of connections to accept from the kernel
		// per socket event. If there are more than
		// MaxAcceptPerSocketEvent connections pending accept,
		// connections over
		// this threshold will be accepted in later event loop iterations.
		// Defaults to 1 and can be disabled by setting to 0 for allowing
		// unlimited accepted connections.
		maxAcceptPerSocketEvent?: int32
	...
	headers?: {
		...
		earlyRequestHeaders?: {
			...
			// GET /foo HTTP/1.1
			// my-header2: bar
			remove?: list.MaxItems(64) & [...string]
	...
	// ProxyProtocol configures the Proxy Protocol settings. When
	// configured,
	// the Proxy Protocol header will be interpreted and the Client
	// Address
	// will be added into the X-Forwarded-For header.
	// If both EnableProxyProtocol and ProxyProtocol are set,
	// ProxyProtocol takes precedence.
	proxyProtocol?: struct.MinFields(0) & {
		// Optional allows requests without a Proxy Protocol header to be
		// proxied.
		// If set to true, the listener will accept requests without a
		// Proxy Protocol header.
		// If set to false, the listener will reject requests without a
		// Proxy Protocol header.
		// If not set, the default behavior is to reject requests without
		// a Proxy Protocol header.
		// Warning: Optional breaks conformance with the specification.
		// Only enable if ALL traffic to the listener comes from a
		// trusted source.
		// For more information on security implications, see
		// haproxy.org/download/2.1/doc/proxy-protocol.txt
		optional?: bool
	}
	...
	timeout?: {
		...
		http?: {
			...
			// The stream idle timeout defines the amount of time a stream can
			// exist without any upstream or downstream activity.
			// Default: 5 minutes.
			streamIdleTimeout?: =~"^([0-9]{1,5}(h|m|s|ms)){1,4}$"
		}
		tcp?: {
	...
	tls?: {
		...
		clientValidation?: {
			...
			// An optional list of hex-encoded SHA-256 hashes. If specified,
			// Envoy will
			// verify that the SHA-256 of the DER-encoded presented
			// certificate matches
			...
			// An optional list of base64-encoded SHA-256 hashes. If
			// specified, Envoy will
			// verify that the SHA-256 of the DER-encoded Subject Public Key
			// Information
			// (SPKI) of the presented certificate matches one of the
			// specified values.
			spkiHashes?: [...string]

			// An optional list of Subject Alternative name matchers. If
			// specified, Envoy
			// will verify that the Subject Alternative Name of the presented
			// certificate
