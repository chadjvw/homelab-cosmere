...
#EnvoyProxySpec: {
	...
	// LuaValidation determines strictness of the Lua script
	// validation for Lua EnvoyExtensionPolicies
	// Default: Strict
		...
		kubernetes?: {
			...
			envoyDaemonSet?: {
				...
				container?: {
					...
					// including a tag, instead of the default image.
					// This field is mutually exclusive with ImageRepository.
					image?: string

					// ImageRepository specifies the container image repository to be
					// used without specifying a tag.
	...
	shutdown?: {
		...
		drainTimeout?: =~"^([0-9]{1,5}(h|m|s|ms)){1,4}$"
		...
		minDrainDuration?: =~"^([0-9]{1,5}(h|m|s|ms)){1,4}$"
	}
	...
	telemetry?: {
		...
		tracing?: {
			...
			provider!: {
				...
				// ServiceName defines the service name to use in tracing
				// configuration.
				// If not set, Envoy Gateway will use a default service name set
				// as
				// "name.namespace" (e.g., "my-gateway.default").
				// Note: This field is only supported for OpenTelemetry and
				// Datadog tracing providers.
				// For Zipkin, the service name in traces is always derived from
				// the Envoy --service-cluster flag
				// (typically "namespace/name" format). Setting this field has no
				// effect for Zipkin.
				serviceName?: string
