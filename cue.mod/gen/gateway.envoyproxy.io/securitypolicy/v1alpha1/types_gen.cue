...
//timoni:generate timoni mod vendor crd -f https://github.com/envoyproxy/gateway/releases/download/v1.5.1/envoy-gateway-crds.yaml
...
#SecurityPolicySpec: {
	...
	apiKeyAuth?: {
		...
		// ForwardClientIDHeader is the name of the header to forward the
		// client identity to the backend
		// service. The header will be added to the request with the
		// client id as the value.
		forwardClientIDHeader?: string

		// Sanitize indicates whether to remove the API key from the
		// request before forwarding it to the backend service.
		sanitize?: bool
	}
	...
	authorization?: {
		...
		rules?: [...{
			...
			principal!: {
				...
				// You can use the `ClientIPDetection` or the `ProxyProtocol`
				// field in
	...
	extAuth?: {
		...
		//
		// If set to true, the External Authorization will also be
		// bypassed if its configuration is invalid.
		failOpen?: bool
		...
		grpc?: {
			...
			backendSettings?: {
				...
				dns?: {
					...
					dnsRefreshRate?: =~"^([0-9]{1,5}(h|m|s|ms)){1,4}$"
				...
				healthCheck?: {
					...
					active?: {
						...
						// InitialJitter defines the maximum time Envoy will wait before
						// the first health check.
						// Envoy will randomly select a value between 0 and the initial
						// jitter value.
						initialJitter?: =~"^([0-9]{1,5}(h|m|s|ms)){1,4}$"
					...
					passive?: {
						...
						baseEjectionTime?: =~"^([0-9]{1,5}(h|m|s|ms)){1,4}$"
				...
				loadBalancer?: {
					...
					endpointOverride?: {
						// ExtractFrom defines the sources to extract endpoint override
						// information from.
						extractFrom!: list.MaxItems(10) & [...{
							// Header defines the header to get the override endpoint
							// addresses.
							// The header value must specify at least one endpoint in
							// `IP:Port` format or multiple endpoints in
							// `IP:Port,IP:Port,...` format.
							// For example `10.0.0.5:8080` or
							// `[2600:4040:5204::1574:24ae]:80`.
					...
					zoneAware?: {
						// PreferLocalZone configures zone-aware routing to prefer sending
						// traffic to the local locality zone.
						preferLocal?: {
							force?: {
								// MinEndpointsInZoneThreshold is the minimum number of upstream
								// endpoints in the local zone required to honor the
								// forceLocalZone
								// override. This is useful for protecting zones with fewer
								// endpoints.
								minEndpointsInZoneThreshold?: int32
							}

							// MinEndpointsThreshold is the minimum number of total upstream
							// endpoints across all zones required to enable zone-aware
							// routing.
							minEndpointsThreshold?: int64
						}
					}
				}
				proxyProtocol?: {
				...
				retry?: {
					...
					perRetry?: {
						...
						backOff?: {
							...
							baseInterval?: =~"^([0-9]{1,5}(h|m|s|ms)){1,4}$"
							...
							maxInterval?: =~"^([0-9]{1,5}(h|m|s|ms)){1,4}$"
						}
	...
	oidc?: {
		...
		// The Kubernetes secret which contains the client ID to be used
		// in the
		// [Authentication
		// Request](https://openid.net/specs/openid-connect-core-1_0.html#AuthRequest).
		// Exactly one of clientID or clientIDRef must be set.
		// This is an Opaque secret. The client ID should be stored in the
		// key "client-id".
		//
		// Only one of clientID or clientIDRef must be set.
		clientIDRef?: {
		...
		defaultRefreshTokenTTL?: =~"^([0-9]{1,5}(h|m|s|ms)){1,4}$"
		...
		defaultTokenTTL?: =~"^([0-9]{1,5}(h|m|s|ms)){1,4}$"
		denyRedirect?: {
		...
		provider!: {
			...
			//
			// If the end session endpoint is provided, EG will use it to log
			// out the user from the OIDC Provider when the user accesses the
			// logout path.
			// EG will also try to discover the end session endpoint from the
